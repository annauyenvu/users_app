...
class _HomePageState extends State<HomePage> {
  ...
  bool requestinDirectionDetailsInfo = false;
  ...
  displayUserRideDetailsContainer() async  {
    //draw route between pickup and dropOffDestination
    ///Directions API
  ...
  retrieveDirectionDetails() async {
    ...
    var dropOffGeoGraphicCoOrdinates = LatLng(dropOffDestinationLocation!.latitudePosition!, dropOffDestinationLocation.longitudePosition!);
    ...
    List<PointLatLng> latLngPointsFromPickUpToDestination = pointsPolyline.decodePolyline(tripDirectionDetailsInfo!.encodedPoints!); //list of latlng
    ...
    if(pickupGeoGraphicCoOrdinates.latitude >dropOffGeoGraphicCoOrdinates.latitude && pickupGeoGraphicCoOrdinates.longitude > dropOffGeoGraphicCoOrdinates.longitude) {
    ...
    Circle dropOffDestinationPointCircle = Circle(
      circleId: const CircleId("dropOffDestinationPointCircle"),
      strokeColor: Colors.blue,
      strokeWidth: 4,
      radius: 14,
        ...
        .listen((driverEvent) {
          if(driverEvent != null) {
            ...
            switch (onlineDriverChild) {
              case Geofire.onKeyEntered: //enter the radius
              ...
              case Geofire.onKeyExited: //exit the redius
                ManageDriversMethods.removeDriverFromList(driverEvent["key"]);
                //update drivers on google map
                updateAvailableNearbyOnlineDriversOnMap();
                break;

              case Geofire.onKeyMoved: //moving inside radius
                OnlineNearbyDrivers onlineNearbyDrivers = OnlineNearbyDrivers();
                onlineNearbyDrivers.uidDriver = driverEvent["key"];
                onlineNearbyDrivers.latDriver = driverEvent["latitude"];
                onlineNearbyDrivers.lngDriver = driverEvent["longitude"];
    ...
    tripStreamSubscription = tripRequestRef!.onValue.listen((eventSnapshot) async {
      ...
      if((eventSnapshot.snapshot.value as Map)["driverLocation"] != null) {
        ...
        if(status == "accepted") {
          ...
          //update info for drop off to user in UI
      ...
      if(status == "accepted") {
        ...
        //remove drivers markers after a driver accept the trip
        setState(() {
  ...
  updateFromDriverCurrentLocationToPickUp(driverCurrentLocationLatLng) async {
    if(!requestinDirectionDetailsInfo) {
      requestinDirectionDetailsInfo = true;
      ...
      setState(() {
        tripStatusDisplay = "Driver is Coming - ${directionDetailsPickUp.durationTextString} min - ${directionDetailsPickUp.distanceTextString} km";
      });
      requestinDirectionDetailsInfo = false;
    }
  }
  ...
  updateFromDriverCurrentLocationToDropOffDestination(driverCurrentLocationLatLng) async {
    if(!requestinDirectionDetailsInfo) {
      ...
      setState(() {
        tripStatusDisplay = "Driving to Drop Off Location- ${directionDetailsPickUp.durationTextString} min";
      });
      requestinDirectionDetailsInfo = false;
    }
  }
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    ...
                                    ? "d ${(cMethods.calculateFareAmount(tripDirectionDetailsInfo!)).toString()}"
                                    :"",
                                    style: const TextStyle(
                                      fontSize: 18,
                                      color: Colors.white70,
                    ...
                    Row(
                      ...
                      children: [
                        ClipOval(
                          child: Image.network(
                            photoDriver == ""
                                ? "https://firebasestorage.googleapis.com/v0/b/taxi-dispatch-21caf.appspot.com/o/avatarman.png?alt=media&token=e08f7248-bb03-4cdd-aa70-e43593b6b3af"
                                : photoDriver,
                            width: 60,
                            height: 60,
                            fit:  BoxFit.cover,
